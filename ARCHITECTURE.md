...
## 17. 画像運用ルール（視覚標準・2026-07-08確立）

### 17.1 役割分担（最重要）
- **雰囲気・比喩＝自作生成イラスト**（ブランドパレット: パイングリーン/生成り/柿/ゴールド・文字なし）
- **正確さが要る比較・図解＝HTML/CSS製**（例: SubsidySteps.astro）。画像生成は日本語文字が崩れるため、文字入り図解を画像で作らない
- 他社ロゴ・公式スクショは**ASP提携後の許諾済み素材のみ**。X投稿は公式埋め込み（oEmbed）のみ可

### 17.2 配置の標準（全ページ・全記事）
- スクールカード/個別ページ: `public/img/genres/ai-school/{id}.jpg`（fs存在チェックで未生成校は自動スキップ→生成したら置くだけで表示）
- セグメントLP冒頭: `seg-{slug}.jpg` / 口コミアバター: `avatar-1..5.jpg`（性別・職業と矛盾しない割当=Voices.astroのpickAvatar）
- ブログ: 冒頭=frontmatter `heroImage`（**未指定ならblog-publisherが自動生成して付与**）・中間figure=`<figure class="fig">`+figcaption・OGPは全ページBase側でフォールバック済み

### 17.3 生成と最適化
- 生成: Mac mini `eyecatch-gen.py`＋プロンプト正本 `~/.hermes/image-sprint-prompts/`（新規はこの様式を踏襲）
- 最適化必須: `sips -Z 1200 -s format jpeg -s formatOptions 82`（アバターは240px）。**生成PNGをそのままpublicに置かない**（1枚1MB超はCWV毀損）
