 ...
 *  法務監査M-2: 「最大」の限定・受給資格・専門実践の段階合算/年間上限を明示。 */
export const CONDITION_NOTE =
  '給付金・補助金は割引ではありません。講座の修了や転職・就業継続などの条件をすべて達成した後に支給される制度で、達成できない場合は満額負担となります。「最大70%/80%」は上限であり全員が受給できるわけではなく、雇用保険の加入期間など本人の受給資格が必要です（専門実践教育訓練給付金の80%は複数条件の段階達成の合算、上限64万円は年間の上限）。対象要件・支給時期は各制度の公式情報と各スクールの無料相談でご確認ください。';
